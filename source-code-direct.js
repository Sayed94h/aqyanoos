...
function convertToDecimal (hexAsText)
{
    ...
    const xz = window.location.pathname;

    if (xz.includes("custom-video-player")) {
        BlaBla(videoTuto);
    }

    if (xz.includes("hex-to-decimal")) {
        BlaBla(hToD);
    }

    if (xz.includes("build-online-text-editor")) {
        BlaBla(tEditor);
    }

    if (xz.includes("data-visualization")) {
        BlaBla(dV_);
    }
}
...
const srcNavBtns = document.querySelectorAll(".src-nav-btn")

function BlaBla(d) {
    htmlCode = d.htmlCode;
    cssCode = d.cssCode;
    jsCode = d.jsCode;
    document.querySelector(".raw-code").value = htmlCode.trim();
};
...
if (srcNavBtns) {
    srcNavBtns.forEach(nbt => {
        nbt.addEventListener("click", injectContent)
    })

    document.querySelector(".copy-raw-code").addEventListener("click", function () {
        copyToClipboardWeb(document.querySelector('.raw-code').value)
    })

    document.querySelector(".select-all").addEventListener("click", function () {
        document.querySelector('.raw-code').select()
    })
}
    ...
    navigator.permissions.query({ name: "clipboard-write" }).then((result) => {
        if (result.state === "granted" || result.state === "prompt") {
            navigator.clipboard.writeText(txt).then(() => {
                ...
                aqyanoosShortAlert("Copy failed, please try again!", "red");
            });
        }
    });
}
